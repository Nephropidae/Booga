...
public class Unfriend extends PluginCommand {
            ...
            if (args.length > 1) {
                try {
                    ...
                    if (Main.playerExtensions.get(p).friends.contains(friend.getUniqueId().toString())) {
                        Main.playerExtensions.get(p).friends.remove(friend.getUniqueId().toString());
                        Main.plugin.getDataConfig().set("players." + p.getUniqueId().toString() + ".friends",
                                Main.playerExtensions.get(p).friends);
                        p.sendMessage("You've removed " + friend.getDisplayName() + " from your friends list.");
                    } else {
                        p.sendMessage(friend.getDisplayName() + " isn't on your friends list!");
                    }
                } catch (Exception e) {
                    p.sendMessage("Could not execute command: did you enter a player's name?");
                }
            } else {
                p.sendMessage("Wrong usage! /booga unfriend <friend name>");
            }
        }
    }
}
